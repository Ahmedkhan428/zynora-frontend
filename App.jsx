...
function App() {
  ...
  // --- LIMIT & USAGE STATES (100 Chats, 10 Images, 24 Hours Cooldown) ---
  ...
  const API_URL = 'https://zynora-backend-production-8f2f.up.railway.app';
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#0c0718", color: "white", overflow: "hidden" }}>
      {/* Sidebar */}
      <div style={{ width: "260px", background: "#110a24", borderRight: "1px solid rgba(255,255,255,0.08)", display: "flex", flexDirection: "column", padding: "16px" }}>
        <div style={{ fontSize: "18px", fontWeight: "bold", marginBottom: "20px", color: "#c084fc" }}>✦ Zynora AI</div>
        
        {/* New Chat Button */}
        <button onClick={() => { setActiveView('chat'); setCurrentSessionId(null); setMessages([]); }}
          style={{ ...sharedButtonStyle, opacity: activeView === 'chat' ? 1 : 0.85 }}>
          + New Chat
        </button>

        {/* Redeem Code Button */}
        <button onClick={() => setActiveView('redeem')}
          style={{ ...sharedButtonStyle, opacity: activeView === 'redeem' ? 1 : 0.85, marginBottom: "20px" }}>
          🔑 Redeem Code
        </button>

        {/* Usage Tracker info for normal users */}
        {!isUnlimited && (
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
        {activeView === 'redeem' ? (
          <>
            <div style={{ padding: "18px 24px", borderBottom: "1px solid rgba(255,255,255,0.08)", fontSize: "16px", fontWeight: "600" }}>✦ Zynora AI Redeem Center</div>
          ...
          <>
            <div style={{ padding: "18px 24px", borderBottom: "1px solid rgba(255,255,255,0.08)", fontSize: "16px", fontWeight: "600" }}>✦ Zynora AI Chat</div>
